...
import { getNextMonthYear, getPreviousMonthDate, getPreviousYearDate, } from '../../utils/date';
...
test('Create a subscription', async ({ page }) => {
  ...
  await page.fill('#ffInput_date', getPreviousMonthDate());
  await page.locator('#ffInput_repeat_freq').selectOption('monthly');
  await page.fill('#ffInput_bill_end_date' ,getPreviousYearDate());
